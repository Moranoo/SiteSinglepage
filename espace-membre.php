...
$mysqli=mysqli_connect('localhost','root','','steno');//'serveur','nom d'utilisateur','pass','nom de la table'
if(!$mysqli) {
...
<html>
	...
	<body>
	<a href="index.html"><img class="mb-4" src="img/logo.jpg" alt="logo de Sténotypie style="width=82 height=82 style="margin-left:30%"></a>
				...
				if(isset($_POST['valider'])){
					if(!isset($_POST['mail'])){
						...
						if(!preg_match("#^[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?@[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?\.[a-z]{2,30}$#i",$_POST['mail'])){
							//cette preg_match est un petit peu complexe
